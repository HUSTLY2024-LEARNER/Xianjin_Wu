...
def convert_yolo_to_coco(yolo_dir, coco_dir):
    images = []
    annotations = []
    categories = [{"id": 0, "name": "cars"}]  # 修改为你的类别信息
    ...
    for split in ["train", "val"]:
        ...
        coco_images_path = os.path.join(coco_dir, f"{split}")
    ...
    instances_train_path = os.path.join(annotations_path, "train.json")
    instances_val_path = os.path.join(annotations_path, "val.json")
...
# 使用示例
yolo_dataset_directory = 'datasets/full_field'
coco_dataset_directory = 'datasets/coco'
